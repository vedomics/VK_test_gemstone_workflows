...
workflow SKA_plasmid_masked {
  meta {
    ...
    description: "SKA analysis with plasmid masking using MOB-suite"
  }
  ...
  Array[File] all_plasmid_fastas = flatten(mob_recon.plasmid_fastas)

  scatter (plasmid_fasta in all_plasmid_fastas) {
    call plasmid_to_skf {
      input:
  ...
  scatter (i in range(length(samplename))) {
    call mask_genomes {
      input:
        unmasked_skf = SKA1_build.skf_file[i],
        merged_plasmid_skf = merge_plasmid_skf.merged_skf,
        sample_id = samplename[i]
    }
  }
...
task merge_plasmid_skf {
  input {
    Array[File] plasmid_skf_files
    String strain
  }

  String merged_name = "~{strain}_all_plasmids_merged"
